...
# The normalization values are different from MNIST since SVHN is in color
transform = transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize((0.4377, 0.4438, 0.4728), (0.1980, 0.2010, 0.1970))  # SVHN mean and std for RGB channels
])

# Load SVHN datasets
train_dataset = datasets.SVHN(
    root='./data',
    split='train',  # SVHN uses 'split' parameter instead of 'train'
    download=True,
    transform=transform
)
...
# Create data loaders with the same batch size
...
class CNN(nn.Module):
    def __init__(self):
        ...
        # Input: 32x32x3 -> Output: 32x32x32 (after padding)
        self.conv_block1 = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=3, padding=1),  # Changed input channels from 1 to 3 for RGB
            nn.BatchNorm2d(32),  # Added batch normalization for better training stability
            nn.ReLU(),
            nn.Conv2d(32, 32, kernel_size=3, padding=1),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2),  # Output: 16x16x32
            nn.Dropout(0.25)
        )
        ...
        # Note: Input size is larger due to larger input images (32x32 vs 28x28)
        self.fc_layers = nn.Sequential(
            nn.Linear(4096, 4096),  # First fully connected layer
            nn.BatchNorm1d(4096),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(4096, 512),  # Second fully connected layer
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Linear(512, 10)  # Output layer - 10 classes for SVHN
        )
    ...
    def _register_hooks(self):
        ...
        Properly counts ReLU layers and stores their activations.
        """
        ...
        def get_activation_hook(name):
            def hook(module, input, output):
                ...
                if name not in self.layer_activations:
                    self.layer_activations[name] = []
                self.layer_activations[name] = output.detach().cpu()

            return hook

        # Count and register ReLU layers
        self.num_relu_layers = 0
        for name, module in self.named_modules():
            if isinstance(module, nn.ReLU):
                self.num_relu_layers += 1
                hook = module.register_forward_hook(
                    get_activation_hook(f'relu_{self.num_relu_layers}')
                )
...
class LayerwiseVisualization:
    ...
    def _register_hooks(self):
        ...
        def get_activation_hook(name):
            def hook(module, input, output):
                # For convolutional layers, we need to properly reshape the output
                if len(output.shape) == 4:
                    # Reshape from (batch_size, channels, height, width) to (batch_size, channels*height*width)
                    batch_size = output.size(0)
                    flattened = output.view(batch_size, -1)
                    self.layer_activations[name] = flattened.detach().cpu()
                else:
                    # For fully connected layers, just store the output
                    self.layer_activations[name] = output.detach().cpu()
    ...
    def visualize_layer_comparisons(self, data_loader, num_samples=2000):
        ...
        for idx, (layer_name, activations) in enumerate(layer_data.items(), 1):
            print(f"Processing layer {layer_name} with shape {activations.shape}")

            # Compute t-SNE projection with proper error handling
            try:
                tsne = TSNE(n_components=2, random_state=42)
...
def get_layer_activations(model, data_loader, num_samples=2000):
    """
    Get activations from the model's layers for visualization.
    ...
    Returns:
        tuple: (activations array, labels array)
    """
    model.eval()
    activations = []
    labels = []
    ...
    with torch.no_grad():
        for data, target in data_loader:
            ...
            if last_relu_key in model.layer_activations and len(model.layer_activations[last_relu_key]) > 0:
                activation_batch = model.layer_activations[last_relu_key]
                if isinstance(activation_batch, list):
                    activation_batch = activation_batch[-1]
                activation_batch = activation_batch[:batch_size]
...
# Initialize model, criterion, and optimizer with adjusted parameters for SVHN
model = CNN().to(device)
criterion = nn.CrossEntropyLoss()

# Use Adam optimizer for better convergence on SVHN
