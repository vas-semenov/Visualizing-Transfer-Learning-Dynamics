...
def get_layer_activations(model, data_loader, num_samples=2000):
    """Collect activations from a specific layer for visualization"""
    model.eval()
    activations = []
    labels = []
    count = 0
...
def get_all_layer_activations(model, data_loader, num_samples=2000):
    """Collect activations from all layers for visualization"""
    model.eval()
    ...
    with torch.no_grad():
        for data, target in data_loader:
            ...
            # Store activations from all ReLU layers
            for layer_num in range(1, model.num_relu_layers + 1):
                layer_key = f'relu_{layer_num}'
                if model.layer_activations[layer_key]:
                    activation_batch = model.layer_activations[layer_key][-1][:batch_size]
                    layer_activations[layer_key].append(activation_batch.cpu().numpy())
    ...
    for layer_name, acts in layer_activations.items():
        if acts:  # Check if we collected any activations
            processed_activations[layer_name] = np.vstack(acts)

    return processed_activations, np.array(labels)
...
def create_comprehensive_analysis(trainer, test_loader):
    """Creates a comprehensive analysis of the network's internal representations"""
