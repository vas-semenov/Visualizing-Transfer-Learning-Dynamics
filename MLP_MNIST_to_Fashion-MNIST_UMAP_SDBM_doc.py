...
# Suppress specific warnings related to UMAP
...
class SSNP:
    ...
    def fit(self, X, y=None, epochs=100, batch_size=32, verbose=0):
        ...
        # Use UMAP to create initial projections that respect class structure
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            if y is not None:
                # Supervised projection with labels
                reducer = UMAP(n_components=2, n_neighbors=15, min_dist=0.1,
                               random_state=42, target_metric='categorical',
                               target_weight=0.5)
                projections = reducer.fit_transform(X_scaled, y)
            else:
...
def create_sdbm_decision_boundary_map(model, train_loader, test_loader, epoch,
    ...
    with torch.no_grad():
        for i in range(0, synthetic_high_dim.shape[0], batch_size):
            ...
            # For MLP, we need to pass through just the final layer
            outputs = model.layers[-1](batch_tensor)
    ...
    plt.contourf(xx, yy, Z, alpha=boundary_alpha, cmap='tab10')

    # Plot scatter points (true labels)
    scatter = plt.scatter(test_projected[:, 0], test_projected[:, 1],
                          c=test_labels, s=20, edgecolor='k', cmap='tab10')
...
def visualize_sdbm_source_target_boundaries(transfer_manager, source_loader, target_loader, epoch,
    ...
    for idx, ((data, labels), (loader_name, dataset_name)) in enumerate(zip(data_collections, loaders)):
        ...
        # Plot decision regions
        plt.contourf(xx, yy, Z, alpha=0.6, cmap='tab10')

        # Plot scatter points
        scatter = plt.scatter(embedded_data[:, 0], embedded_data[:, 1],
                              c=labels, s=15, edgecolor='k', cmap='tab10')
...
def get_penultimate_features(model, data):
    ...
    # Process through the model layers up to the penultimate layer
    for i in range(len(model.layers) - 1):  # Skip the final layer
        x = model.layers[i](x)
...
class AdvancedVisualization:
    ...
    def visualize_epoch_evolution(self, epochs_to_show=None, max_plots_per_figure=12,
        ...
        for i in range(0, len(epochs_to_show), max_plots_per_figure):
            ...
            # Create UMAP projections for each epoch in this batch
            for idx, epoch in enumerate(batch_epochs, 1):
                ...
                with warnings.catch_warnings():
                    ...
                    projections = umap_reducer.fit_transform(data['activations'])
    ...
    def visualize_neuron_relationships(self, dataset_name="", phase=""):
        ...
        if self.neuron_correlations is None:
            raise ValueError("Need to compute neuron correlations first")
...
class LayerwiseVisualization:
    ...
    def visualize_layer_comparisons(self, data_loader, num_samples=1000,
        ...
        """
        # Get activations from all layers with memory efficiency parameter
        layer_data, labels = self.get_layer_representations(
            data_loader, num_samples, memory_efficient=memory_efficient
        )
        ...
        # Create UMAP projection for each layer
        for idx, (layer_name, activations) in enumerate(layer_data.items(), 1):
            ...
            # Compute UMAP projection with suppressed warnings
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                umap_reducer = UMAP(n_components=2, n_neighbors=15, min_dist=0.1, random_state=42)
                projections = umap_reducer.fit_transform(activations)
...
class TransferLearningManager:
    ...
    def fine_tune(self, train_loader, test_loader, epochs=100,
                  layers_to_freeze=None, activation_capture_frequency=10,  # Changed from 5 to 10
                  learning_rate_reduction=0.1, target_input_size=784,
                  reinit_layers=[0, 1], lr_scheduler_type='cosine',
                  visualization_samples=1000,  # Reduced sample count for visualization
                  memory_optimization=True):  # New parameter
        """
        ...
        Args:
            ...
            visualization_samples: Number of samples to use for visualizations
            memory_optimization: Whether to use memory optimization
        """
        print(f"Fine-tuning model for {self.target_dataset_name} with extended training and memory optimization...")

        # Adapt model for the target dataset with selective layer retention
        self.prepare_model_for_target_dataset(target_input_size, reinit_layers)

        # Save initial model state for comparison
        initial_state = copy.deepcopy(self.model.state_dict())

        # Adjust initial learning rate for fine-tuning
        ...
        for epoch in range(epochs):
            ...
            if epoch % activation_capture_frequency == 0 or epoch == epochs - 1:
                ...
                # Add the decision boundary visualization using SDBM
    ...
    def _visualize_source_target_layer_comparison(self, source_activations, source_labels,
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            umap_reducer = UMAP(n_components=2, n_neighbors=15, min_dist=0.1, random_state=42)
            source_proj = umap_reducer.fit_transform(source_activations)
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            umap_reducer = UMAP(n_components=2, n_neighbors=15, min_dist=0.1, random_state=42)
            target_proj = umap_reducer.fit_transform(target_activations)
    ...
    def clear_memory(self):
        ...
        if hasattr(self, 'layer_viz') and hasattr(self.layer_viz, 'layer_activations'):
    ...
    def prepare_model_for_target_dataset(self, input_size=784, reinit_layers=[0, 1]):
        """
        Selectively retain knowledge from source model by reinitializing specified layers.

        This approach keeps knowledge from middle and later layers while allowing early layers
        to adapt more freely to the new domain.

        Args:
            input_size: Input dimension for target dataset (default: 784 for MNIST/Fashion-MNIST)
            reinit_layers: List of layer indices to reinitialize (layer 0 is first hidden layer)
        """
        print(f"Adapting model for target dataset with selective layer retention")
        print(f"Reinitializing layers: {reinit_layers}")

        # Get the current state dict to preserve weights for non-reinitialized layers
        state_dict = self.model.state_dict()

        # Reinitialize specified layers
        for layer_idx in reinit_layers:
            # Calculate the actual module indices (each layer has weight, bias, etc.)
            layer_base_idx = layer_idx * 3  # Each layer consists of Linear + ReLU + Dropout

            # Get corresponding layer
            if layer_idx == 0:
                # First layer handling
                first_layer_output_size = self.model.layers[0].out_features
                new_layer = nn.Linear(input_size, first_layer_output_size)
                self.model.layers[0] = new_layer
                print(f"  Layer {layer_idx}: Reinitialized first layer to handle input size {input_size}")
            else:
                # Handle other layers - reinitialize but keep same dimensions
                layer_linear_idx = layer_base_idx
                try:
                    current_layer = self.model.layers[layer_linear_idx]
                    in_features = current_layer.in_features
                    out_features = current_layer.out_features

                    new_layer = nn.Linear(in_features, out_features)
                    self.model.layers[layer_linear_idx] = new_layer
                    print(f"  Layer {layer_idx}: Reinitialized ({in_features} → {out_features})")
                except IndexError:
                    print(f"  Layer {layer_idx}: Index out of range, skipping")

        # Initialize all reinitialized layers with kaiming initialization
        for m in self.model.modules():
            if isinstance(m, nn.Linear) and not hasattr(m, 'initialized_from_source'):
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                nn.init.constant_(m.bias, 0)

        # Load the saved weights for non-reinitialized layers
        ...
        # Mark layers that were kept from source model
        kept_layers = []
        for name in filtered_state_dict.keys():
            if 'layers' in name and 'weight' in name:
                parts = name.split('.')
                if len(parts) >= 2:
                    layer_num = int(parts[1]) // 3
                    if layer_num not in reinit_layers:
                        kept_layers.append(layer_num)

        kept_layers = sorted(list(set(kept_layers)))
        print(f"Keeping knowledge from layers: {kept_layers}")

        # Update model with preserved weights
        model_dict.update(filtered_state_dict)
        self.model.load_state_dict(model_dict)

        # Mark preserved layers to avoid reinitializing them again
        for idx in kept_layers:
            layer_base_idx = idx * 3
            if layer_base_idx < len(self.model.layers):
                self.model.layers[layer_base_idx].initialized_from_source = True

        print("Model adaptation complete with selective layer retention")
...
if __name__ == '__main__':
    ...
    """
    # Define Fashion-MNIST data loaders (MNIST loaders are already defined above)
    fashion_transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.2860,), (0.3530,))  # Fashion-MNIST specific mean and std
    ])
    ...
    # Fashion-MNIST has the same input size as MNIST (784 pixels)
    fashion_input_size = 784

    # Fine-tune with selective layer retention and extended training
    print("\nFine-tuning on Fashion-MNIST with memory optimization...")
    transfer_manager.fine_tune(
        fashion_train_loader,
        fashion_test_loader,
        epochs=70,  # Reduced from 100 to 70 based on your logs showing convergence by epoch 63
        layers_to_freeze=None,
        activation_capture_frequency=10,  # Increased from 5 to 10
        target_input_size=fashion_input_size,
        reinit_layers=[0, 1],
        lr_scheduler_type='cosine',
        visualization_samples=1000,  # Reduced sample count
